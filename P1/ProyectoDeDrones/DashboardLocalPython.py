...
def onTheGround ():
    ...
    takeOffBtn['text'] = 'On the ground'
    takeOffBtn.after(1500, lambda: takeOffBtn.config(text='Despegar'))
    takeOffBtn['fg'] = 'black'
...
def land ():
    ...
    dron.Land(blocking = False, callback = onTheGround)
    landBtn['text'] = 'Landing'
    landBtn['fg'] = 'white'
    landBtn['bg'] = 'green'
...
def RTL():
    global dron
    # llamada no bloqueante
    dron.RTL(blocking = False, callback = onTheGround2)
    RTLBtn['text'] = 'Going back home'
    RTLBtn['fg'] = 'white'
    RTLBtn['bg'] = 'green'
...
def gotodone (DoneBtn):
    DoneBtn.config(text = 'On place')
    DoneBtn['fg'] = 'white'
    DoneBtn['bg'] = 'green'

def goto (LatEntry, LonEntry, AltEntry,boton_goto):
    global dron
    try:
        lat = float(LatEntry.get())
        lon = float(LonEntry.get())
        alt = float(AltEntry.get())
        boton_goto.config(text = 'On my way')
        dron.goto (lat, lon, alt, blocking = False, callback = gotodone, params = boton_goto)
    except ValueError:
        boton_goto.config(text = 'Error' )
        boton_goto['bg'] = 'red'
        boton_goto.after(1000, lambda: boton_goto.config(text='Go to'))
        boton_goto['bg'] = 'dark orange'
...
def crear_ventana():
    global dron
    global  altShowLbl, headingShowLbl,  speedSldr, gradesSldr, stateShowLbl, SpeedShowLbl, ModeShowLbl
    ...
    speedSldr = tk.Scale(ventana, label="Velocidad (m/s):", resolution=1, from_=0, to=20, tickinterval=5,orient=tk.HORIZONTAL)
    ...
    ModeLbl = tk.Label(telemetryFrame, text='Mode')
    ...
    # Botones para lat y longitud
    LatEntry = tk.Entry(ventana)
    LatEntry.grid(row=9, column=0, padx=5, pady=5)

    LonEntry = tk.Entry(ventana)
    LonEntry.grid(row=9, column=1, padx=5, pady=5)

    AltEntry = tk.Entry(ventana)
    AltEntry.grid(row=9, column=2, padx=5, pady=5)

    GotoBtn = tk.Button(ventana, text="Go to", bg="dark orange")
    GotoBtn.config (command = lambda: goto(LatEntry, LonEntry, AltEntry, GotoBtn))
    GotoBtn.grid(row=9, column=3, padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)
