...
def changeHeading (event):
    global dron
    global gradesSldr
    value = int (gradesSldr.get())
    client.publish('interfazGlobal/autopilotServiceDemo/changeHeading', str(value))


def changeNavSpeed (event):
    global dron
    global speedSldr
    value = float (speedSldr.get())
    client.publish('interfazGlobal/autopilotServiceDemo/changeNavSpeed', str(value))
...
def on_message(client, userdata, message):
    ...
    if message.topic == 'Grup2/autopilotServiceDemo/interfazGlobal/telemetryInfo':
    ...
    if message.topic == 'Grup2/autopilotServiceDemo/interfazGlobal/connected':
        connectBtn['text'] = 'Conectado'
        connectBtn['fg'] = 'white'
        connectBtn['bg'] = 'green'


    if message.topic == 'Grup2/autopilotServiceDemo/interfazGlobal/flying':
        arm_takeOffBtn['text'] = 'En el aire'
        arm_takeOffBtn['fg'] = 'white'
        arm_takeOffBtn['bg'] = 'green'

    if message.topic == 'Grup2/autopilotServiceDemo/interfazGlobal/landed':
        landBtn['text'] = 'En tierra'
        landBtn['fg'] = 'white'
        landBtn['bg'] = 'green'
        restart()
    if message.topic == 'Grup2/autopilotServiceDemo/interfazGlobal/atHome':
...
def crear_ventana():
    ...
    client.subscribe('Grup2/autopilotServiceDemo/interfazGlobal/#')
    client.loop_start()
