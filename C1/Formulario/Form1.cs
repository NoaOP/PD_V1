...
namespace Formulario
{
    public partial class Form1 : Form
    {
        ...
        private void ProcesarTelemetria(byte id, List<(string nombre, float valor)> telemetria)
        {
            ...
            // --- Dades bàsiques que JA arriben de la DLL ---
            ...
            if (lastLatDeg.HasValue && lastLonDeg.HasValue && lastTime.HasValue)
            {
                ...
                if (dt > 0.05) // evita valors bojos si arriben massa ràpid
                {
                    double dLat = lat - lastLatDeg.Value;
                    double dLon = lon - lastLonDeg.Value;

                    // Conversió ràpida graus -> metres (aprox local)
        ...
        {
            // Recojo y muestro el valor la altura según se mueve 
            // la barra de desplazamiento
        ...
        {
            // Botón para subir 5 metros: el dron sube 5m y continúa con lo que estaba haciendo
            try
            {
                // Verificamos que tengamos datos de telemetría para saber la altitud inicial
                if (!string.IsNullOrEmpty(altitudLbl.Text))
                {
                    ...
                    // Iniciamos la subida usando navegación "Up"
                    dron.Navegar("Up");
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        int intentos = 0;
                        int maxIntentos = 100; // Máximo 10 segundos (100 * 100ms)
                        ...
                        // Si alcanzamos el tiempo máximo sin llegar, detenemos de todos modos
                ...
                {
                    MessageBox.Show("No hay datos de telemetría disponibles.\n\nPara usar esta función:\n1. Conecta el dron\n2. Despega\n3. Inicia la telemetría", 
                        "Información", 
