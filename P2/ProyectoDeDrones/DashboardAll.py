...
MAVPROXY_LOCAL_ENDPOINT = os.getenv("MAVPROXY_LOCAL_ENDPOINT", "udp:127.0.0.1:14550")
MAVPROXY_LOCAL_BAUD = int(os.getenv("MAVPROXY_LOCAL_BAUD", "115200"))
...
class DashboardAllApp:
    ...
    def _build_shell(self):
        ...
        self.local_mode_btn = tk.Button(
            header, text="Mode Local", bg=self.local_color, fg="white",
            command=lambda: self.switch_mode("local"))
        self.local_mode_btn.grid(row=0, column=0, padx=4, pady=4,
                                 sticky=tk.N + tk.S + tk.E + tk.W)

        self.global_mode_btn = tk.Button(
            header, text="Mode Global", bg=self.local_color, fg="white",
            command=lambda: self.switch_mode("global"))
        self.global_mode_btn.grid(row=0, column=1, padx=4, pady=4,
                                  sticky=tk.N + tk.S + tk.E + tk.W)

        self.mode_label = tk.Label(header, text="")
        self.mode_label.grid(row=0, column=2, padx=4, pady=4,
                             sticky=tk.N + tk.S + tk.E + tk.W)

        self.content_frame = tk.Frame(self.root)
        self.content_frame.grid(row=1, column=0,
                                sticky=tk.N + tk.S + tk.E + tk.W, padx=8, pady=8)
    ...
    def _build_map_section(self, parent):
        ...
        map_frame.grid(row=0, column=0, padx=5, pady=5,
                       sticky=tk.N + tk.S + tk.E + tk.W)
        ...
        if MAP_DEPS_OK:
            ...
            self.map_status_label = tk.Label(map_frame,
                                             text="Click al mapa per enviar el dron")
            ...
            self.map_status_label.grid(row=0, column=0, padx=5, pady=5,
                                       sticky=tk.N + tk.S + tk.E + tk.W)
    ...
    def _on_map_click(self, coordinates):
        ...
        if self.mode == "local":
            ...
            if None in (lat_entry, lon_entry, alt_entry, goto_btn):
                self._safe_widget_config(self.map_status_label,
                                         text="Go To local no disponible")
                return
            ...
            self._safe_widget_config(self.map_status_label,
                                     text=f"Go to local: {lat:.6f}, {lon:.6f}")
            return

        if self.mode == "global":
            if not self._global_autopilot_available():
                self._safe_widget_config(self.map_status_label,
                                         text="Autopilot global tancat")
                return
            self.global_map_target = (float(lat), float(lon))
            self._safe_widget_config(self.map_status_label,
                                     text=f"Objectiu global: {lat:.6f}, {lon:.6f}")
    ...
    def _update_map_from_telemetry(self, telemetry_info):
        ...
        if len(self.drone_path_positions) > 1:
            if self.map_path_line is not None:
                try:
                    ...
                    pass
            self.map_path_line = self.map_widget.set_path(
                self.drone_path_positions, color="blue")
        ...
        self.map_marker = self.map_widget.set_marker(
            lat, lon, icon=self.current_rotated_icon)
    ...
    def _drive_global_to_map_target(self, telemetry_info):
        ...
        if telemetry_info.get("state") != "flying":
            self._safe_widget_config(self.map_status_label,
                                     text="Objectiu guardat, esperant vol")
            return
        target_lat, target_lon = self.global_map_target
        direction, reached = self._get_global_go_direction(
        ...
        self._safe_widget_config(self.map_status_label,
                                 text=f"Anant cap a objectiu ({direction})")

    # ---------------------------------------------------------- service status

    def _refresh_service_status_labels(self):
        local_info_label = self.local_widgets.get("services_info_label")
        self._safe_widget_config(
            local_info_label,
            text=(f"Autopilot: {'OBERT' if self.autopilot_status else 'TANCAT'} | "
                  f"Camera: {'OBERT' if self.camera_status else 'TANCAT'}"),
            fg="green" if (self.autopilot_status or self.camera_status) else "red")

        self._safe_widget_config(
            self.global_widgets.get("autopilot_status_label"),
            text=f"Autopilot: {'OBERT' if self.autopilot_status else 'TANCAT'}",
            fg="green" if self.autopilot_status else "red")

        self._safe_widget_config(
            self.global_widgets.get("camera_status_label"),
    ...
    def _update_service_status_from_system(self):
        autopilot_on = (self.autopilot_process is not None
                        and self.autopilot_process.poll() is None)
        camera_on    = (self.camera_process is not None
    ...
    def _manual_start_autopilot(self):
        self.autopilot_process, _ = self._start_service_process(
            "AutopilotService.py", self.autopilot_process, show_console=True)
        self._update_service_status_from_system()

    def _manual_start_camera(self):
        self.camera_process, _ = self._start_service_process(
            "CameraService.py", self.camera_process, show_console=True)
        self._update_service_status_from_system()
    ...
    def switch_mode(self, mode):
        ...
        if mode == "local" and not self.local_lock_owned:
            if not _acquire_single_instance_lock():
                messagebox.showwarning("Mode local ocupat",
    ...
    def _cleanup_mode(self):
        # Aturar telemetria ABANS de destruir widgets per evitar race conditions
    ...
    def _start_service_process(self, script_name, current_process, show_console=False):
        ...
        try:
            if os.name == "nt":
                creationflags = (subprocess.CREATE_NEW_CONSOLE if show_console
    ...
    def _start_local_services(self):
        failed_services = []
        self.autopilot_process, autopilot_ok = self._start_service_process(
            "AutopilotService.py", self.autopilot_process)
        if not autopilot_ok:
            failed_services.append("autopilot")

        self.camera_process, camera_ok = self._start_service_process(
            "CameraService.py", self.camera_process)
        if not camera_ok:
        ...
        if failed_services:
            self.mode_label.config(
                text=f"Actiu: Local (error: {', '.join(failed_services)})")
    ...
    def _build_local_view(self):
        ...
        connect_btn = tk.Button(left_panel, text="Conectar",
                                bg=self.local_color, fg="white",
                                command=self._local_connect)
        connect_btn.grid(row=0, column=0, padx=5, pady=5,
                         sticky=tk.N + tk.S + tk.E + tk.W)

        arm_btn = tk.Button(left_panel, text="Armar",
                            bg=self.local_color, fg="white",
                            command=self._local_arm)
        arm_btn.grid(row=1, column=0, padx=5, pady=5,
                     sticky=tk.N + tk.S + tk.E + tk.W)

        takeoff_btn = tk.Button(left_panel, text="Despegar",
                                bg=self.local_color, fg="white",
                                command=self._local_takeoff)
        takeoff_btn.grid(row=2, column=0, padx=5, pady=5,
                         sticky=tk.N + tk.S + tk.E + tk.W)

        land_btn = tk.Button(left_panel, text="Aterrizar",
                             bg=self.local_color, fg="white",
                             command=self._local_land)
        land_btn.grid(row=3, column=0, padx=5, pady=5,
                      sticky=tk.N + tk.S + tk.E + tk.W)

        rtl_btn = tk.Button(left_panel, text="RTL",
                            bg=self.local_color, fg="white",
                            command=self._local_rtl)
        ...
        goto_frame.grid(row=1, column=0, padx=5, pady=5,
                        sticky=tk.N + tk.S + tk.E + tk.W)
        goto_frame.columnconfigure(0, weight=1)
        goto_frame.columnconfigure(1, weight=1)

        tk.Label(goto_frame, text="Lat").grid(row=0, column=0, padx=3, pady=3,
                                              sticky=tk.W)
        lat_entry = tk.Entry(goto_frame, width=10)
        lat_entry.grid(row=0, column=1, padx=3, pady=3,
                       sticky=tk.N + tk.S + tk.E + tk.W)

        tk.Label(goto_frame, text="Lon").grid(row=1, column=0, padx=3, pady=3,
                                              sticky=tk.W)
        lon_entry = tk.Entry(goto_frame, width=10)
        lon_entry.grid(row=1, column=1, padx=3, pady=3,
                       sticky=tk.N + tk.S + tk.E + tk.W)

        tk.Label(goto_frame, text="Alt").grid(row=2, column=0, padx=3, pady=3,
                                              sticky=tk.W)
        alt_entry = tk.Entry(goto_frame, width=10)
        alt_entry.grid(row=2, column=1, padx=3, pady=3,
                       sticky=tk.N + tk.S + tk.E + tk.W)

        goto_btn = tk.Button(
            goto_frame, text="Go to", bg=self.local_color, fg="white",
            command=lambda: self._local_goto(lat_entry, lon_entry, alt_entry, goto_btn))
        goto_btn.grid(row=3, column=0, columnspan=2, padx=5, pady=5,
                      sticky=tk.N + tk.S + tk.E + tk.W)

        video_btn = tk.Button(right_panel, text="Recibir video por WebRTC",
                              bg=self.local_color, fg="white",
                              command=self._local_start_video)
        video_btn.grid(row=2, column=0, padx=5, pady=5,
                       sticky=tk.N + tk.S + tk.E + tk.W)

        detect_frame = tk.LabelFrame(right_panel, text="Deteccion de objects")
        detect_frame.grid(row=3, column=0, padx=5, pady=5,
                          sticky=tk.N + tk.S + tk.E + tk.W)
        for idx in range(4):
        ...
        for idx, (label, object_id) in enumerate(COCO_SUBSET):
            ...
            tk.Checkbutton(detect_frame, text=label, variable=var,
                           anchor="w").grid(row=idx // 4, column=idx % 4,
                                            padx=2, pady=2,
                                            sticky=tk.N + tk.S + tk.E + tk.W)

        tk.Button(detect_frame, text="Sin deteccion",
                  bg=self.local_color, fg="white",
                  command=lambda: self._clear_detection_selection("local")).grid(
            row=(len(COCO_SUBSET) // 4) + 1, column=0, columnspan=4,
            padx=2, pady=4, sticky=tk.N + tk.S + tk.E + tk.W)

        services_info_label = tk.Label(right_panel, text="", anchor="w")
        services_info_label.grid(row=4, column=0, padx=5, pady=5,
                                 sticky=tk.N + tk.S + tk.E + tk.W)

        # MAP COLUMN
        map_panel = tk.Frame(panel)
        map_panel.grid(row=0, column=2, sticky=tk.N + tk.S + tk.E + tk.W,
                       padx=5, pady=5)
        map_panel.rowconfigure(0, weight=1)
        map_panel.columnconfigure(0, weight=1)
        self._build_map_section(map_panel)

        # Guardem tots els widgets — NO sobreescrivim els de telemetria creats per _build_local_telemetry
        self.local_widgets.update({
    ...
    def _build_global_view(self):
        ...
        self.global_client.on_disconnect = self._global_on_disconnect
        self.global_mqtt_connected = False
        ...
        services_frame.grid(row=1, column=0, padx=5, pady=8,
                            sticky=tk.N + tk.S + tk.E + tk.W)
        services_frame.columnconfigure(0, weight=1)
        services_frame.columnconfigure(1, weight=1)

        open_autopilot_btn = tk.Button(
            services_frame, text="Obrir AutopilotService",
            bg=self.global_color, fg="white",
            command=self._manual_start_autopilot)
        open_autopilot_btn.grid(row=0, column=0, padx=5, pady=4,
                                sticky=tk.N + tk.S + tk.E + tk.W)

        open_camera_btn = tk.Button(
            services_frame, text="Obrir CameraService",
            bg=self.global_color, fg="white",
            command=self._manual_start_camera)
        open_camera_btn.grid(row=0, column=1, padx=5, pady=4,
                             sticky=tk.N + tk.S + tk.E + tk.W)

        autopilot_status_label = tk.Label(services_frame,
                                          text="Autopilot: TANCAT", fg="red")
        autopilot_status_label.grid(row=1, column=0, padx=5, pady=2, sticky=tk.W)

        camera_status_label = tk.Label(services_frame,
                                       text="Camera: TANCAT", fg="red")
        camera_status_label.grid(row=1, column=1, padx=5, pady=2, sticky=tk.W)

        global_video_btn = tk.Button(
            right_panel, text="Recibir video por WebRTC",
            bg=self.global_color, fg="white",
            command=self._global_start_video)
        global_video_btn.grid(row=2, column=0, padx=5, pady=5,
                              sticky=tk.N + tk.S + tk.E + tk.W)

        global_detect_frame = tk.LabelFrame(right_panel, text="Deteccion de objects")
        global_detect_frame.grid(row=3, column=0, padx=5, pady=5,
                                 sticky=tk.N + tk.S + tk.E + tk.W)
        for idx in range(4):
        ...
        for idx, (label, object_id) in enumerate(COCO_SUBSET):
            ...
            tk.Checkbutton(global_detect_frame, text=label, variable=var,
                           anchor="w").grid(row=idx // 4, column=idx % 4,
                                            padx=2, pady=2,
                                            sticky=tk.N + tk.S + tk.E + tk.W)

        tk.Button(global_detect_frame, text="Sin deteccion",
                  bg=self.global_color, fg="white",
                  command=lambda: self._clear_detection_selection("global")).grid(
            row=(len(COCO_SUBSET) // 4) + 1, column=0, columnspan=4,
            padx=2, pady=4, sticky=tk.N + tk.S + tk.E + tk.W)
    ...
    def _build_nav_buttons(self, nav_frame, go_handler):
        ...
        buttons = [
            ("NW", "NorthWest", 0, 0), ("No", "North", 0, 1), ("NE", "NorthEast", 0, 2),
            ("We", "West",  1, 0),     ("St", "Stop",  1, 1), ("Ea", "East",  1, 2),
            ("SW", "Down",  2, 0),     ("So", "South", 2, 1), ("SE", "Up",    2, 2),
        ]
        for text, direction, row, col in buttons:
            color = self.global_color if self.mode == "global" else self.local_color
            btn = tk.Button(nav_frame, text=text, bg=color, fg="white")
    ...
    def _build_local_telemetry(self, parent):
        ...
        for idx, text in enumerate(labels):
            tk.Label(parent, text=text).grid(row=0, column=idx, padx=5, pady=5,
                                             sticky=tk.N + tk.S + tk.E + tk.W)
        # Guardem directament al diccionari de widgets
    ...
    def _start_video(self, mode):
        if not VIDEO_DEPS_OK:
            ...
            btn = widgets.get("video_btn")
            if btn is not None:
                btn.config(text="Faltan deps video", bg="red", fg="white")
                btn.after(1500, lambda: self._safe_widget_config(
                    btn, text="Recibir video por WebRTC", bg=color, fg="white"))
            return

        if mode == "local":
            if self.local_video_thread is not None and self.local_video_thread.is_alive():
                self._safe_widget_config(self.local_widgets.get("video_btn"),
                                         text="Video activo", bg="green", fg="white")
                return
            self.local_video_stop_event = threading.Event()
            self.local_video_thread = threading.Thread(
                target=self._local_video_thread_runner, daemon=True)
            self.local_video_thread.start()
            self._safe_widget_config(self.local_widgets.get("video_btn"),
                                     text="Video activo", bg="green", fg="white")
        else:
            if self.global_video_thread is not None and self.global_video_thread.is_alive():
                self._safe_widget_config(self.global_widgets.get("video_btn"),
                                         text="Video activo", bg="green", fg="white")
                return
            self.global_video_stop_event = threading.Event()
            self.global_video_thread = threading.Thread(
                target=self._global_video_thread_runner, daemon=True)
    ...
    async def _video_receiver(self, mode):
        ...
        receiver     = LocalVideoReceiver(
            lambda: self._get_selected_object_ids(mode), stop_event, window_title)
    ...
    def _local_on_connected(self):
        """Callback cridat quan la connexió MAVProxy s'estableix (no bloquejant)."""
        self._safe_widget_config(self.local_widgets.get("connect_btn"),
                                 text="Conectado MAVProxy", fg="white", bg="green")
        self._safe_widget_config(self.local_widgets.get("speed_sldr"), state="normal")
        if self.local_widgets.get("speed_sldr"):
            self.local_widgets["speed_sldr"].set(1)

    def _local_connect(self):
        # Crear instància nova i connectar de forma NO BLOQUEJANT per no congelar la UI
        self.local_dron = Dron()
        self._safe_widget_config(self.local_widgets.get("connect_btn"),
                                 text="Connectant...", fg="black", bg="orange")
        try:
            self.local_dron.connect(
                MAVPROXY_LOCAL_ENDPOINT,
                MAVPROXY_LOCAL_BAUD,
                blocking=False,
                callback=lambda: self._run_on_ui_thread(self._local_on_connected)
            )
        except Exception as ex:
            self._safe_widget_config(self.local_widgets.get("connect_btn"),
                                     text="Error conexion", fg="white", bg="red")
            messagebox.showerror(
                "Error connexió local",
                f"No s'ha pogut connectar via MAVProxy a {MAVPROXY_LOCAL_ENDPOINT}.\n\n{ex}")

    def _local_arm(self):
        self.local_dron.arm()
        self._safe_widget_config(self.local_widgets.get("arm_btn"),
                                 text="Armado", fg="white", bg="green")

    def _local_in_the_air(self):
        self._safe_widget_config(self.local_widgets.get("takeoff_btn"),
                                 text="En el aire", fg="white", bg="green")

    def _local_takeoff(self):
        self.local_dron.takeOff(5, blocking=False, callback=self._local_in_the_air)
        self._safe_widget_config(self.local_widgets.get("takeoff_btn"),
                                 text="Despegando...", fg="black", bg="yellow")

    def _local_on_ground_land(self):
        takeoff_btn = self.local_widgets.get("takeoff_btn")
        self._safe_widget_config(self.local_widgets.get("land_btn"),
                                 text="On the ground", fg="white", bg=self.local_color)
        self._safe_widget_config(takeoff_btn,
                                 text="On the ground", fg="white", bg=self.local_color)
        if takeoff_btn:
            self.root.after(1500, lambda: self._safe_widget_config(
                takeoff_btn, text="Despegar"))

    def _local_on_ground_rtl(self):
        takeoff_btn = self.local_widgets.get("takeoff_btn")
        self._safe_widget_config(self.local_widgets.get("rtl_btn"),
                                 text="On the ground", fg="white", bg=self.local_color)
        self._safe_widget_config(takeoff_btn,
                                 text="On the ground", fg="white", bg=self.local_color)
        if takeoff_btn:
    ...
    def _local_land(self):
        ...
        self._safe_widget_config(self.local_widgets.get("land_btn"),
                                 text="Landing", fg="white", bg="green")

    def _local_rtl(self):
        self.local_dron.RTL(blocking=False, callback=self._local_on_ground_rtl)
        self._safe_widget_config(self.local_widgets.get("rtl_btn"),
                                 text="Going back home", fg="white", bg="green")

    def _local_go(self, direction, btn):
        if self.local_previous_btn is not None:
            self._safe_widget_config(self.local_previous_btn,
                                     fg="white", bg=self.local_color)
        self.local_dron.go(direction)
        self._safe_widget_config(btn, fg="white", bg="green")
        self.local_previous_btn = btn

    def _local_goto_done(self, done_btn):
        self._safe_widget_config(done_btn, text="On place", fg="white", bg="green")
    ...
    def _local_goto(self, lat_entry, lon_entry, alt_entry, goto_btn):
        try:
            ...
            self._safe_widget_config(goto_btn, text="On my way")
            self.local_dron.goto(lat, lon, alt, blocking=False,
                                 callback=self._local_goto_done, params=goto_btn)
        except ValueError:
            self._safe_widget_config(goto_btn, text="Error", bg="red")
            self.root.after(1000, lambda: self._safe_widget_config(
                goto_btn, text="Go to", bg=self.local_color))

    def _local_start_telem(self):
        self.local_dron.send_telemetry_info(
            lambda telemetry: self._run_on_ui_thread(
                self._local_show_telemetry, telemetry))

    def _local_stop_telem(self):
        self.local_dron.stop_sending_telemetry_info()
        for key in ("alt_show", "heading_show", "state_show", "speed_show", "mode_show"):
            self._safe_widget_config(self.local_widgets.get(key), text="--")

    def _local_change_heading(self, _event):
        sldr = self.local_widgets.get("heading_sldr")
        if sldr:
            self.local_dron.changeHeading(int(sldr.get()))

    def _local_change_speed(self, _event):
        sldr = self.local_widgets.get("speed_sldr")
        if sldr:
            self.local_dron.changeNavSpeed(float(sldr.get()))
    ...
    def _global_autopilot_available(self):
        # In global mode (Laptop 2), commands must depend on MQTT/LAN reachability,
        # not on local processes running in this same machine.
        return self.global_client is not None and self.global_mqtt_connected
    ...
    def _global_land(self):
        if not self._global_autopilot_available():
            self._safe_widget_config(self.global_widgets.get("land_btn"),
                                     text="Autopilot tancat", fg="white", bg="red")
            return
        self._global_publish("interfazGlobal/autopilotServiceDemo/Land")
        self._safe_widget_config(self.global_widgets.get("land_btn"),
                                 text="Aterrizando...", fg="black", bg="yellow")

    def _global_rtl(self):
        if not self._global_autopilot_available():
            self._safe_widget_config(self.global_widgets.get("rtl_btn"),
                                     text="Autopilot tancat", fg="white", bg="red")
            return
        self._global_publish("interfazGlobal/autopilotServiceDemo/RTL")
        self._safe_widget_config(self.global_widgets.get("rtl_btn"),
                                 text="Retornando...", fg="black", bg="yellow")

    def _global_go(self, direction, btn):
        if not self._global_autopilot_available():
            return
        if self.global_previous_btn is not None:
            self._safe_widget_config(self.global_previous_btn,
                                     fg="white", bg=self.global_color)
        self._global_publish("interfazGlobal/autopilotServiceDemo/go", direction)
        self._safe_widget_config(btn, fg="white", bg="green")
    ...
    def _global_stop_telem(self):
        ...
        for key in ("alt_show", "heading_show", "state_show"):
            self._safe_widget_config(self.global_widgets.get(key), text="--")

    def _global_change_heading(self, _event):
        sldr = self.global_widgets.get("heading_sldr")
        if sldr:
            self._global_publish("interfazGlobal/autopilotServiceDemo/changeHeading",
                                 str(int(sldr.get())))

    def _global_change_speed(self, _event):
        sldr = self.global_widgets.get("speed_sldr")
        if sldr:
            self._global_publish("interfazGlobal/autopilotServiceDemo/changeNavSpeed",
                                 str(float(sldr.get())))

    def _global_on_connect(self, _client, _userdata, _flags, rc):
        if rc == 0:
            self.global_mqtt_connected = True
            print("MQTT connected OK, rc=", rc)
        else:
            self.global_mqtt_connected = False
            print("MQTT bad connection, rc=", rc)

    def _global_on_disconnect(self, _client, _userdata, rc):
        self.global_mqtt_connected = False
    ...
    def _global_reset_buttons(self):
        self._safe_widget_config(self.global_widgets.get("arm_takeoff_btn"),
                                 text="Despegar", fg="white", bg=self.global_color)
        self._safe_widget_config(self.global_widgets.get("land_btn"),
                                 text="Aterrizar", fg="white", bg=self.global_color)
        self._safe_widget_config(self.global_widgets.get("rtl_btn"),
                                 text="RTL", fg="white", bg=self.global_color)
        if self.global_previous_btn is not None:
            self._safe_widget_config(self.global_previous_btn,
                                     fg="white", bg=self.global_color)

    def _global_on_message(self, _client, _userdata, message):
        """Rebut en thread MQTT — tot accés a widgets via _run_on_ui_thread + _safe_widget_config."""
        topic = message.topic
        ...
        if topic == "Grup2/autopilotServiceDemo/interfazGlobal/telemetryInfo":
        ...
        elif topic == "Grup2/autopilotServiceDemo/interfazGlobal/connected":
            ...
            self._run_on_ui_thread(
                self._safe_widget_config,
                self.global_widgets.get("arm_takeoff_btn"),
                text="En el aire", fg="white", bg="green")

        elif topic == "Grup2/autopilotServiceDemo/interfazGlobal/landed":
            self._run_on_ui_thread(
                self._safe_widget_config,
                self.global_widgets.get("land_btn"),
                text="En tierra", fg="white", bg="green")
            self.root.after(5000, self._global_reset_buttons)

        elif topic == "Grup2/autopilotServiceDemo/interfazGlobal/atHome":
            self._run_on_ui_thread(
                self._safe_widget_config,
                self.global_widgets.get("rtl_btn"),
                text="En tierra", fg="white", bg="green")
            self.root.after(5000, self._global_reset_buttons)
...
if __name__ == "__main__":
    ...
    if args.mode == "local":
        if not _acquire_single_instance_lock():
            ...
            messagebox.showwarning("Dashboard ja obert",
                                   "Ja hi ha una instancia del DashboardAll en Local oberta.")
    ...
    main_root = tk.Tk()
    app = DashboardAllApp(main_root, start_mode=args.mode,
                          owns_local_lock=owns_local_lock)
    main_root.mainloop()
