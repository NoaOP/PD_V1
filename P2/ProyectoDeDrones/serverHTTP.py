...
TOPIC_TELEMETRY_SUB = "Grup2/autopilotServiceDemo/mobileFlask/telemetryInfo"  # donde viene telemetría
...
def on_message(client, userdata, msg):
    try:
        ...
        with telemetry_lock:
            # Intentamos leer 'alt', si no existe buscamos 'altitude'
            telemetry["alt"] = float(data.get("alt") if data.get("alt") is not None else data.get("altitude", 0.0))
            telemetry["state"] = data.get("state", "unknown")
            telemetry["flightMode"] = data.get("flightMode", "UNKNOWN")
