...
def showTelemetryInfo (telemetry_info):
    ...
    global altShowLbl, headingShowLbl, stateShowLbl, speedShowLbl, modeShowLbl
    ...
    modeShowLbl['text'] = telemetry_info['flightMode']
...
def goto_cmd():
    global dron
    try:
        lat = float(latEnt.get())
        lon = float(lonEnt.get())
        alt = float(altEnt.get())
    except:
        gotoBtn['text'] = 'Error coords'
        gotoBtn['fg'] = 'white'
        gotoBtn['bg'] = 'red'
        return
    # llamada NO bloqueante, con callback
    dron.goto(lat, lon, alt, blocking=False, callback=arrivedToGoto)
    gotoBtn['text'] = 'Yendo...'
    gotoBtn['fg'] = 'black'
    gotoBtn['bg'] = 'yellow'

def inTheGroundFromLand():
    landBtn['text'] = 'En tierra'
    landBtn['fg'] = 'white'
    landBtn['bg'] = 'green'

def inTheGroundFromRTL():
...
def land ():
    ...
    dron.Land(blocking=False, callback=inTheGroundFromLand)
...
def RTL():
    global dron
    # llamada NO bloqueante
    dron.RTL(blocking=False, callback=inTheGroundFromRTL)
...
def crear_ventana():
    ...
    global connectBtn, armBtn, takeOffBtn, landBtn, RTLBtn, gotoBtn
    #GOTO
    global latEnt, lonEnt, altEnt
    ...
    telemetryFrame.columnconfigure(4, weight=1)
    ...
    modeLbl = tk.Label(telemetryFrame, text='Modo')
    modeLbl.grid(row=0, column=4, padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)
    ...
    modeShowLbl = tk.Label(telemetryFrame, text='')
    modeShowLbl.grid(row=1, column=4, padx=5, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)

    # GOTO
    gotoFrame = tk.LabelFrame(ventana, text="GoTo")
    gotoFrame.grid(row=9, column=0, columnspan=2, padx=10, pady=5, sticky=tk.N + tk.S + tk.E + tk.W)

    gotoFrame.columnconfigure(0, weight=1)
    gotoFrame.columnconfigure(1, weight=1)
    gotoFrame.columnconfigure(2, weight=1)
    gotoFrame.columnconfigure(3, weight=1)

    tk.Label(gotoFrame, text="Lat").grid(row=0, column=0, padx=3, pady=2)
    tk.Label(gotoFrame, text="Lon").grid(row=0, column=1, padx=3, pady=2)
    tk.Label(gotoFrame, text="Alt").grid(row=0, column=2, padx=3, pady=2)

    latEnt = tk.Entry(gotoFrame)
    latEnt.grid(row=1, column=0, padx=3, pady=3, sticky=tk.N + tk.S + tk.E + tk.W)

    lonEnt = tk.Entry(gotoFrame)
    lonEnt.grid(row=1, column=1, padx=3, pady=3, sticky=tk.N + tk.S + tk.E + tk.W)

    altEnt = tk.Entry(gotoFrame)
    altEnt.grid(row=1, column=2, padx=3, pady=3, sticky=tk.N + tk.S + tk.E + tk.W)

    gotoBtn = tk.Button(gotoFrame, text="GoTo", bg="dark orange", command=goto_cmd)
    gotoBtn.grid(row=1, column=3, padx=3, pady=3, sticky=tk.N + tk.S + tk.E + tk.W)
    return ventana
