     ...
     # afegit per la simulacio de la web app
        ...
        if dron.state in ("flying", "armed", "connected"):
            try:
                ...
                # 2. Le decimos al dron que la inicie de forma no bloqueante (para no colgar MQTT)
                dron.executeMission(blocking=False)

            except json.JSONDecodeError:
                print("❌ Error: JSON de waypoints mal formateado.")
            except Exception as e:
                print(f"❌ Error al ejecutar Misión: {e}")

    elif command == "stop_rtl":
        if dron.state in ("flying", "armed"):
            print("🚨 EMERGENCIA RECIBIDA: Abortando misión y ejecutando RTL 🚨")

            # Al llamar a RTL, el autopiloto (ArduPilot/MAVLink) interrumpe automáticamente
            # cualquier misión AUTO que esté ejecutando y pasa a modo RTL.
            dron.RTL(blocking=False, callback=lambda p: publish_event(origin, p), params="atHome")
